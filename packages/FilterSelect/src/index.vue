...
export default {
  ...
  methods: {
    load () {
      ...
      const own = column.own
      let itemRender = own && (own.cellRender || own.editRender)
      const option = column.filters[0]
      const { vals } = option.data
      const optionsData = Array.isArray(itemRender.options) ? itemRender.options : []
      const colValList = optionsData.map(item => {
        return {
          checked: vals.includes(item.label),
          value: item.label
        }
      })
